...
use crate::nix_emit_model::RenderedPackagePlan;
...
pub(crate) fn append_crate_plan_section(
    out: &mut String,
    rendered_packages: &[RenderedPackagePlan],
) {
    ...
    for package in rendered_packages {
        let _ = writeln!(
            ...
            nix_string_list(&package.workspace_source_prefixes),
            nix_string_list(&package.target_triples),
            nix_bool(package.needs_host_artifacts),
            nix_escape(&package.command_script),
        );
    }
    out.push_str("  ];\n");
}
