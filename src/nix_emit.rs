...
pub fn render_nix_expression(plan: &Plan, release_mode: bool) -> String {
    ...
    out.push_str("            local cwd program entry key value status outDir crateName outputPath\n");
    ...
    out.push_str("            status=$?\n");
    out.push_str("            if [ \"$status\" -eq 0 ] && [ \"''${crateName}\" = \"build_script_build\" ] && [ -n \"$outputPath\" ] && [ -x \"$outputPath\" ]; then\n");
