...
pub fn render_nix_expression(plan: &Plan, release_mode: bool) -> String {
    ...
    out.push_str("            local cwd program entry key value status outDir manifestDir runDir crateName outputPath commandOutDir\n");
    ...
    out.push_str("                (cd \"$runDir\" && env \"''${envArgs[@]}\" OUT_DIR=\"$outDir\" CARGO_MANIFEST_DIR=\"$runDir\" \"''${nixcargo_last_build_script_binary}\")\n");
    out.push_str("              else\n");
    out.push_str("                env \"''${envArgs[@]}\" OUT_DIR=\"$outDir\" CARGO_MANIFEST_DIR=\"$runDir\" \"''${nixcargo_last_build_script_binary}\"\n");
    out.push_str("              fi\n");
